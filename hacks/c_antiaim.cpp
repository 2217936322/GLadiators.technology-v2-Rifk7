...
void c_antiaim::fakelag(c_cs_player* local, c_user_cmd* cmd, bool& send_packet)
{
	...
	static constexpr auto target_standing = 8;
	static constexpr auto target_moving = 12;
	static constexpr auto target_air = 12;
	static constexpr auto target_slowwalk = 8;
	auto target_standing_custom = config.misc.fakelag_settings.fake_lag_standing;
	auto target_moving_custom = config.misc.fakelag_settings.fake_lag_moving;
	auto target_air_custom = config.misc.fakelag_settings.fake_lag_air;
	auto target_slowwalk_custom = config.misc.fakelag_settings.fake_lag_slowwalk;
	...
	auto choke_amount = config.misc.fakelag_settings.fake_lag_automatic ? target_standing : target_standing_custom;
	if (!local || !local->is_alive() || !cmd || game_rules->is_freeze_period() || !config.rage.enabled) // —делал включение с рейджом, ибо без фейклагов анти-паста в автостопе работает :( 
	{
		choke_amount = 0;
		estimated_choke = 0;
		choking_amt = 0;
	...
	if (local->get_velocity().length2d() > info->get_standing_accuracy(weapon) || local->get_duck_amount() > 0.f)
		choke_amount = config.misc.fakelag_settings.fake_lag_automatic ? target_moving : target_moving_custom;
	...
	{
		choke_amount = config.misc.fakelag_settings.fake_lag_automatic ? target_slowwalk : target_slowwalk_custom;
	}
	if (!local->is_on_ground() && GetAsyncKeyState(config.rage.slow_walk))
		choke_amount = config.misc.fakelag_settings.fake_lag_automatic ? target_air : target_air_custom;

	if (config.misc.fakelag_settings.disable_on_grenade && weapon->is_grenade())
		choke_amount = 1;

	if ((config.misc.fakelag_settings.disable_on_tazer && weapon->get_item_definition() == WEAPON_ZEUS))
		choke_amount = 1;

	if ((config.misc.fakelag_settings.disable_on_knife && weapon->get_item_definition() == WEAPON_KNIFE || config.misc.fakelag_settings.disable_on_knife && weapon->get_item_definition() == WEAPON_KNIFE_T || weapon->get_item_definition() == WEAPON_ZEUS))
		choke_amount = 1;
	...
	if (GetAsyncKeyState(config.misc.fake_duck))
	{
	...
	if (!antiaim->is_fakeducking && weapon->get_item_definition() == WEAPON_REVOLVER && config.misc.fakelag_settings.disable_on_revolver)
		choke_amount = 1;

	if (!antiaim->is_fakeducking && config.misc.fakelag_settings.disable_on_shooting && cmd->buttons & c_user_cmd::attack)
		choke_amount = 1;
...
float c_antiaim::calculate_ideal_yaw(c_cs_player* local, bool estimate)
{
	...
	if (!target.has_value())
		return engine_client()->get_view_angles().y + 178.f; // »значально 180, но фейклаги +30 добавл€ют, € не ебу, походу анти-паста.
...
//			positions[6] = math::rotate_2d(back_pos + c_vector3d(0.f, 0.f, -3.f), current_angle + 90.f, 0.f);
	...
	if (fabsf(lowest_dmg.second - highest_dmg.second) < 20.f && highest_dmg.second <= 40000.f)
		return engine_client()->get_view_angles().y + 180.f; // »значально 180, но фейклаги +30 добавл€ют, € не ебу, походу анти-паста.
	...
	if (fabsf(second_highest_dmg - highest_dmg.second) >= 50000.f)
		return direction.y + highest_dmg.first * step + 180.f; // »значально 180, но фейклаги +30 добавл€ют, € не ебу, походу анти-паста.
...
void c_antiaim::shooting(c_cs_player* local, c_user_cmd* cmd, float ideal_yaw, int direction, bool send_packet)
{
	...
	{
		if (config.misc.fakelag_settings.disable_on_shooting)
