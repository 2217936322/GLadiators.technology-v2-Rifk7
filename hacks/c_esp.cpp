...
void c_esp::draw_nade(nade_info& nade)
{
	...
	renderer->text(origin_screen - c_vector2d(0, 18), get_nade_string(nade.type).c_str(), config.esp.nade_color, fnv1a("visitor_tt12"), esp_flags);
}
...
{
	switch (config.esp.team.impacts)
	{
	case 1:
	{
		// Laser
	...
	case 2:
	{
		...
		beamInfo.model_name = "sprites/blueglow1.vmt";
		beamInfo.model_index = -1; // will be set by CreateBeamPoints if its -1
		beamInfo.halo_scale = 0.0f;
		beamInfo.life = 3.;
		beamInfo.width = 1.5f;
		beamInfo.end_width = 1.5f;
		beamInfo.fade_length = 0.0f;
		beamInfo.amplitude = 0.0f;
		beamInfo.brightness = float(color.alpha);
		beamInfo.speed = 1.0f;
		beamInfo.start_frame = 0;
		beamInfo.frame_rate = 0.f;
		beamInfo.red = float(color.red);
		beamInfo.green = float(color.green);
		beamInfo.blue = float(color.blue);
		beamInfo.segments = 15;
		...
		auto beam = view_render_beams->create_beam_points(beamInfo);
...
{
	renderer->text(from + c_vector2d(width / 2.f, -14.f), player.name, color, fnv1a("visitor_ttESP"), esp_flags);
}
...
void c_esp::esp_health(player_info& player, const c_vector2d& from, const float height, const c_color& color)
{
	...
	renderer->rect_filled_linear_gradient(c_vector2d(from.x - 8, from.y),
		c_vector2d(3, height), c_color(20, 20, 20, 200), c_color(80, 80, 80, 200));
	...
	renderer->rect_filled_linear_gradient(c_vector2d(from.x - 8, from.y + health_scaled),
		c_vector2d(3, height - health_scaled), c_color(255, 255, 255, 200), color);
	...
	renderer->rect(c_vector2d(from.x - 8, from.y),
		c_vector2d(3, height), c_color(0, 0, 0, 255));
}
...
void c_esp::esp_ammo(player_info& player, const c_vector2d& from, const float width, const float height, const c_color& color)
{
	...
	renderer->rect_filled_linear_gradient(c_vector2d(from.x, from.y + height + 8),
		c_vector2d(width, 3), c_color(20, 20, 20, 200), c_color(80, 80, 80, 200));
	...
	// actual health
	renderer->rect_filled_linear_gradient(c_vector2d(from.x, from.y + height + 8),
		c_vector2d(ammo, 3), color, c_color(255, 255, 255, 200), true);

	// black outline
	renderer->rect(c_vector2d(from.x, from.y + height + 8),
		c_vector2d(width, 3), c_color(0, 0, 0, 255));
}
...
void c_esp::esp_weapon(player_info& player, const c_vector2d& from, const float width, const float height, const c_color& color, bool draw_ammo)
{
	...
	if (config.esp.wep_icons)
		...
		renderer->text(c_vector2d(from.x + (width / 2), from.y + height + additive), player.pWeapon->get_weapon_name(), color, fnv1a("visitor_ttESP"), esp_flags);
}
...
void c_esp::draw_player(player_info& player)
{
	...
	if (esp.weapon)
		esp_weapon(player, from, width, height, c_color(esp.weapon_color.red, esp.weapon_color.green, esp.weapon_color.blue, esp.weapon_color.alpha * esp_alpha_fade[idx]), esp.ammo);
