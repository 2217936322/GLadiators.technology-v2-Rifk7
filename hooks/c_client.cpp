...
void __stdcall c_client::create_move(int sequence_number, float input_sample_frametime, bool active, bool& sendpacket)
{
	...
	if (config.chams.local.type && !sendpacket && ~c_user_cmd::use && config.misc.fakelag_settings.enabled)
	{
		if (local->is_alive())
		{
...
//	cmd->viewangles -= local->get_punch_angle() * recoil_scale->get_float();
	...
	*/
	...
	float ratio = (config.misc.aspect_ratio * 0.1) / 2;
	if (ratio > 0.001)
		cvar()->find_var(_("r_aspectratio"))->set_value(ratio);
	else
		cvar()->find_var(_("r_aspectratio"))->set_value((35 * 0.1f) / 2);
}
