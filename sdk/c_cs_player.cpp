...
bool c_cs_player::IsKnifeorNade()
{
	if (!this)
		return false;
	if (!this->is_alive())
	...
	c_base_combat_weapon* pWeapon = (c_base_combat_weapon*)this->get_current_weapon_handle();

	if (!pWeapon)
		return false;

	std::string WeaponName = pWeapon->get_weapon_name();
